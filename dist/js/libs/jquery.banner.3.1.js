define([
	...
	'use strict';	
	function banner(options) { 	//options传入的对象参数
		var that = this;
		...
		this.LOCAL.listMove = function (num, index) {
			...
			options.list.find('span').removeClass("text").eq(index).addClass("text")
		...
		this.LOCAL.btnMove = function (num) {
			...
			options.list.find('span').removeClass("text").eq(that.LOCAL.iNow).addClass("text")
		}
